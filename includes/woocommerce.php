 ...
 * @return void
 */
function prevision_remove_wc_breadcrumbs() {
    remove_action( 'woocommerce_before_main_content', 'woocommerce_breadcrumb', 20, 0 );
}
add_action( 'init', 'prevision_remove_wc_breadcrumbs' );
 ...
 * @return void
 */
function prevision_do_checkout_links() {
	global $woocommerce;
	echo '<a href="' . $woocommerce->cart->get_checkout_url() . '" title="' . __( 'Checkout' ) . '">' . __( 'Checkout' ) . '</a>';
}
add_action( 'genesis_header', 'prevision_do_checkout_links' );

/**
 * Display cart totals
 * @return void
 */
function prevision_do_cart_link() {
	?>
...
add_action( 'genesis_header', 'prevision_do_cart_link' );
